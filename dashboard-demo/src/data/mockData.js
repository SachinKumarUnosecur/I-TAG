...
/**
 * Identity Threat Profile — mapped findings (46 total; per-stage counts vary
 * intentionally rather than sitting flat at 6 each — see the extra block below).
 * Every row TRANSLATES an upstream signal — it invents nothing:
 *   - R/T/E hero arc: jane.doe's `ap-009` shadow-admin hop (ssm → role-deploy-box →
 *     admin:platform) is split into 3 stage-rows (escalation, trust abuse, lateral
 *     pivot into svc-github-actions' deploy-prod-role) per the product case study.
 *   - R also covers priya.sharma (`ap-010`), mark.chen (`ap-014`), svc-payments-api
 *     (`ap-015`), chris.nguyen break-glass (`ap-028`), svc-terraform-apply's
 *     self-authorized OIDC path (`ap-035`).
 *   - A / C cover leavers still live in cloud IAM (alice.brooks, raj.patel,
 *     owen.blake, derek.frost, claire.nguyen) and orphaned NHIs their
 *     departures left behind (svc-old-payments-worker, svc-orphaned-etl,
 *     svc-derek-vault-reader, svc-backup-agent, svc-finance-ghost — created
 *     *after* alice.brooks's termination date, per `ownershipRecords`).
 *   - E covers realized reach (raw PII, KMS blast radius, cross-boundary
 *     transfer, retained delete) already scored by Exposure/Impact.
 *   - P covers enumeration/discovery noise (new joiners, staging→prod
 *     drift, over-permissioned intern) — deliberately low severity so
 *     Risk Profile's `severity !== 'Low'` incident filter still holds.
 *
 * Severity mix: Critical 13 (36%) · High 9 (25%) · Medium 8 (22%) · Low 6 (17%).
 * Matrix coverage (impact:likelihood): desirable 4:1,5:1,3:1 · acceptable
 * 2:3,2:2,1:5,1:4,3:2 · undesirable (majority) · unacceptable 5:3×3 ·
 * catastrophic 5:5,5:4×3,4:4×2 — a real risk register, not an all-red toy.
 * PTRACE counts derive dynamically below via `ptraceFindingCounts`.
 */
export const mitreFindings = [
  // ── P · Probing (Reconnaissance, Discovery) — low severity by design ──────
  { id: "mf-001", technique: "T1087", name: "Account Discovery", tactic: "Discovery", ptraceCategory: "P", identityId: "id-002", identityName: "mark.chen", description: "Repeated IAM enumeration from mark.chen's devops workstation (ListUsers/ListRoles) reaches beyond his GKE-operator scope into production accounts.", severity: "Medium", impact: 2, likelihood: 5, triggeredAt: "2026-07-27T11:00:00Z" },
  { id: "mf-002", technique: "T1087.004", name: "Cloud Account Discovery", tactic: "Discovery", ptraceCategory: "P", identityId: "id-007", identityName: "sara.jones", description: "New-joiner sara.jones generated a ListUsers/ListRoles burst outside her assigned Engineering OU, consistent with account-mapping recon.", severity: "Low", impact: 2, likelihood: 4, triggeredAt: "2026-07-26T13:35:00Z" },
  { id: "mf-003", technique: "T1526", name: "Cloud Service Discovery", tactic: "Discovery", ptraceCategory: "P", identityId: "id-007", identityName: "sara.jones", description: "Unusual ListBuckets volume from sara.jones outside her OU suggests service-level discovery ahead of a targeted access attempt.", severity: "Low", impact: 2, likelihood: 3, triggeredAt: "2026-07-26T13:30:00Z" },
  { id: "mf-004", technique: "T1082", name: "System Information Discovery", tactic: "Discovery", ptraceCategory: "P", identityId: "id-007", identityName: "sara.jones", description: "Low-volume metadata probing on payments-logs from sara.jones shows no follow-on reads yet — early-stage resource mapping.", severity: "Low", impact: 1, likelihood: 5, triggeredAt: "2026-07-23T10:00:00Z" },
  { id: "mf-005", technique: "T1580", name: "Cloud Infrastructure Discovery", tactic: "Discovery", ptraceCategory: "P", identityId: "id-009", identityName: "diego.ramos", description: "DescribeInstances noise from diego.ramos's staging identity reaching into the production account — his GCP-Developers scope shouldn't extend there.", severity: "Low", impact: 1, likelihood: 4, triggeredAt: "2026-07-22T16:40:00Z" },
  { id: "mf-006", technique: "T1526", name: "Cloud Service Discovery", tactic: "Discovery", ptraceCategory: "P", identityId: "id-015", identityName: "kai.sato", description: "Intern kai.sato inherited GCP-Editors at provisioning and is running exploratory BigQuery jobs against prod-analytics well beyond his tenure — Exposure Map already flags the membership as over-permissioned.", severity: "Low", impact: 2, likelihood: 2, triggeredAt: "2026-07-28T09:40:00Z" },

  // ── T · Trust Exploitation (Initial Access, Defense Evasion) ─────────────
  { id: "mf-007", technique: "T1550", name: "Use Alternate Authentication Material", tactic: "Defense Evasion", ptraceCategory: "T", identityId: "id-001", identityName: "jane.doe", description: "The SSM grant on i-0abc123 was scoped for troubleshooting, not administrator trust — jane.doe's session inherits role-deploy-box's instance-profile trust rather than any credential issued to her.", severity: "Critical", impact: 5, likelihood: 4, triggeredAt: "2026-07-31T08:05:00Z" },
  { id: "mf-008", technique: "T1199", name: "Trusted Relationship", tactic: "Initial Access", ptraceCategory: "T", identityId: "id-005", identityName: "alice.brooks", description: "alice.brooks's Okta account was deprovisioned on termination, but her AWS principal still assumes finance-auditor-role — the IdP↔cloud-IAM trust link was never severed.", severity: "Critical", impact: 5, likelihood: 3, triggeredAt: "2026-07-25T09:00:00Z" },
  { id: "mf-009", technique: "T1078.004", name: "Cloud Accounts", tactic: "Defense Evasion", ptraceCategory: "T", identityId: "id-103", identityName: "svc-ci-runner", description: "svc-ci-runner's Azure managed identity carries an Owner assignment on rg-devops-prod that was scoped for CI convenience, not the trust level the role now grants.", severity: "High", impact: 4, likelihood: 3, triggeredAt: "2026-07-27T12:00:00Z" },
  { id: "mf-010", technique: "T1199", name: "Trusted Relationship", tactic: "Initial Access", ptraceCategory: "T", identityId: "id-004", identityName: "tom.walker", description: "tom.walker's AWS-Security-Audit membership is used to assume a cross-account audit role for lateral access outside his normal review scope.", severity: "Medium", impact: 3, likelihood: 4, triggeredAt: "2026-07-21T15:45:00Z" },
  { id: "mf-011", technique: "T1078.004", name: "Cloud Accounts", tactic: "Defense Evasion", ptraceCategory: "T", identityId: "id-008", identityName: "lena.okonkwo", description: "lena.okonkwo's Automation Contributor role lets a runbook assume mi-platform-ops, whose Owner assignment was granted for a one-time migration and never revoked.", severity: "High", impact: 4, likelihood: 2, triggeredAt: "2026-07-30T14:00:00Z" },
  { id: "mf-012", technique: "T1199", name: "Trusted Relationship", tactic: "Initial Access", ptraceCategory: "T", identityId: "id-112", identityName: "svc-github-actions", description: "svc-github-actions authenticates via GitHub OIDC federation into deploy-prod-role — trust scoped for CI deploys that also carries iam:PassRole, extending past its intended blast radius.", severity: "High", impact: 4, likelihood: 2, triggeredAt: "2026-07-28T11:00:00Z" },

  // ── R · Rights Escalation (Privilege Escalation, Credential Access) ──────
  { id: "mf-013", technique: "T1548.002", name: "Abuse Elevation Control Mechanism", tactic: "Privilege Escalation", ptraceCategory: "R", identityId: "id-001", identityName: "jane.doe", description: "jane.doe's ssm:StartSession on i-0abc123 pulls role-deploy-box's credentials via IMDS, escalating an analyst-level grant to AdministratorAccess with no policy ever naming her directly.", severity: "Critical", impact: 5, likelihood: 5, triggeredAt: "2026-07-31T08:00:00Z" },
  { id: "mf-014", technique: "T1134", name: "Access Token Manipulation", tactic: "Privilege Escalation", ptraceCategory: "R", identityId: "id-003", identityName: "priya.sharma", description: "priya.sharma's gcloud SSH session on data-pipeline-vm inherits the VM's Workload Identity, escalating a Data-Analyst grant into iam://data-admin-role with resourcemanager.setIamPolicy.", severity: "Critical", impact: 5, likelihood: 4, triggeredAt: "2026-07-30T11:20:00Z" },
  { id: "mf-015", technique: "T1548", name: "Abuse Elevation Control Mechanism", tactic: "Privilege Escalation", ptraceCategory: "R", identityId: "id-002", identityName: "mark.chen", description: "mark.chen's kubectl exec into a devops-cluster pod inherits the node's service account, escalating GKE-operator access into iam://cluster-admin-role.", severity: "Critical", impact: 4, likelihood: 4, triggeredAt: "2026-07-28T16:00:00Z" },
  { id: "mf-016", technique: "T1548.002", name: "Abuse Elevation Control Mechanism", tactic: "Privilege Escalation", ptraceCategory: "R", identityId: "id-101", identityName: "svc-payments-api", description: "svc-payments-api's EC2 instance profile on i-0pay789 assumes account-poweruser, granting the payments service role far broader iam:* / ec2:* than its application scope requires.", severity: "High", impact: 4, likelihood: 3, triggeredAt: "2026-07-29T10:00:00Z" },
  { id: "mf-017", technique: "T1552.005", name: "Cloud Instance Metadata API", tactic: "Credential Access", ptraceCategory: "R", identityId: "id-013", identityName: "chris.nguyen", description: "chris.nguyen's break-glass ssm:StartSession policy chains into iam://prod-breakglass-admin via EC2 instance profile — sanctioned for incident response, exercised outside any active incident window.", severity: "Medium", impact: 3, likelihood: 3, triggeredAt: "2026-07-29T02:00:00Z" },
  { id: "mf-018", technique: "T1134", name: "Access Token Manipulation", tactic: "Privilege Escalation", ptraceCategory: "R", identityId: "id-109", identityName: "svc-terraform-apply", description: "svc-terraform-apply is self-authorized — lena.okonkwo, its own creator, received elevated access through this same role a day after creating it — and now assumes iam://org-wide-deploy via OIDC + iam:PassRole.", severity: "High", impact: 5, likelihood: 3, triggeredAt: "2026-07-31T01:00:00Z" },

  // ── A · Account Spoofing / Assumption (Initial Access, Defense Evasion) ──
  { id: "mf-019", technique: "T1078", name: "Valid Accounts", tactic: "Initial Access", ptraceCategory: "A", identityId: "id-005", identityName: "alice.brooks", description: "alice.brooks's Workday termination on 2026-06-01 deprovisioned her Okta session, but her AWS IAM user is still live — a departed identity that can still authenticate as herself.", severity: "Critical", impact: 5, likelihood: 4, triggeredAt: "2026-07-28T09:12:00Z" },
  { id: "mf-020", technique: "T1078.004", name: "Cloud Accounts", tactic: "Defense Evasion", ptraceCategory: "A", identityId: "id-006", identityName: "raj.patel", description: "raj.patel's GCP principal continues authenticating against bigquery://legacy-finance more than two months after his Okta deprovisioning — HR, IdP, and cloud IAM disagree on whether this identity still exists.", severity: "Critical", impact: 5, likelihood: 3, triggeredAt: "2026-07-05T00:00:00Z" },
  { id: "mf-021", technique: "T1078.004", name: "Cloud Accounts", tactic: "Initial Access", ptraceCategory: "A", identityId: "id-011", identityName: "owen.blake", description: "owen.blake's IAM user was never disabled after his DevOps departure — dormant, but a live break-glass-equivalent identity an attacker could assume without spoofing anything new.", severity: "Medium", impact: 5, likelihood: 1, triggeredAt: "2026-07-12T19:00:00Z" },
  { id: "mf-022", technique: "T1550.001", name: "Application Access Token", tactic: "Defense Evasion", ptraceCategory: "A", identityId: "id-105", identityName: "svc-old-payments-worker", description: "svc-old-payments-worker's Lambda execution role still decrypts payments secrets years after alice.brooks — its now-departed creator — last touched it; the identity persists exactly as she left it.", severity: "Critical", impact: 5, likelihood: 2, triggeredAt: "2026-07-18T07:40:00Z" },
  { id: "mf-023", technique: "T1078", name: "Valid Accounts", tactic: "Initial Access", ptraceCategory: "A", identityId: "id-021", identityName: "derek.frost", description: "derek.frost's AWS principal remains active seven months after his Security-team departure, and the SecurityScanFull / SecretsReadOnly NHIs he created still resolve back to him as their only originator.", severity: "High", impact: 5, likelihood: 2, triggeredAt: "2026-07-06T00:00:00Z" },
  { id: "mf-024", technique: "T1078.004", name: "Cloud Accounts", tactic: "Defense Evasion", ptraceCategory: "A", identityId: "id-020", identityName: "claire.nguyen", description: "claire.nguyen's Azure account remains enabled four months after her Product-team departure, and svc-claire-support-bridge — the NHI she created — inherited the same unresolved ownership gap.", severity: "High", impact: 4, likelihood: 2, triggeredAt: "2026-07-12T08:00:00Z" },

  // ── C · Concealment & Persistence (Persistence, Defense Evasion) ─────────
  { id: "mf-025", technique: "T1098.001", name: "Additional Cloud Credentials", tactic: "Persistence", ptraceCategory: "C", identityId: "id-104", identityName: "svc-orphaned-etl", description: "svc-orphaned-etl has had no owner since raj.patel's departure, yet its Cloud Function trigger still holds storage.admin — an orphaned identity nobody is actively watching by definition.", severity: "Critical", impact: 4, likelihood: 4, triggeredAt: "2026-07-10T03:00:00Z" },
  { id: "mf-026", technique: "T1098", name: "Account Manipulation", tactic: "Persistence", ptraceCategory: "C", identityId: "id-002", identityName: "mark.chen", description: "mark.chen's kubectl exec pattern lets him create cluster role bindings that persist independent of his own Okta session — access maintained without a standing credential.", severity: "High", impact: 4, likelihood: 3, triggeredAt: "2026-07-28T16:05:00Z" },
  { id: "mf-027", technique: "T1098.001", name: "Additional Cloud Credentials", tactic: "Persistence", ptraceCategory: "C", identityId: "id-129", identityName: "svc-derek-vault-reader", description: "svc-derek-vault-reader has retained SecretsReadOnly since derek.frost's departure — an orphaned-creator NHI with no owner and no attestation cadence to notice continued use.", severity: "High", impact: 4, likelihood: 3, triggeredAt: "2026-06-30T00:00:00Z" },
  { id: "mf-028", technique: "T1098.001", name: "Additional Cloud Credentials", tactic: "Persistence", ptraceCategory: "C", identityId: "id-111", identityName: "svc-backup-agent", description: "svc-backup-agent is suppressed as a break-glass exception, but its nightly EventBridge → Lambda → backup-role chain still holds s3:* / kms:Decrypt on org-backups-prod with no owner since owen.blake's departure.", severity: "Medium", impact: 4, likelihood: 2, triggeredAt: "2026-03-12T03:00:00Z" },
  { id: "mf-029", technique: "T1484", name: "Domain or Tenant Policy Modification", tactic: "Defense Evasion", ptraceCategory: "C", identityId: "id-001", identityName: "jane.doe", description: "jane.doe's privileged Okta group membership allows a conditional-access bypass path never revisited since onboarding — low likelihood of active use, but a standing concealment mechanism if her account is ever compromised.", severity: "Low", impact: 4, likelihood: 1, triggeredAt: "2026-07-15T10:30:00Z" },
  { id: "mf-030", technique: "T1136.003", name: "Create Account: Cloud Account", tactic: "Persistence", ptraceCategory: "C", identityId: "id-115", identityName: "svc-finance-ghost", description: "svc-finance-ghost was created under alice.brooks's credentials after her HR termination date and inherited FinanceReportsRead — a persistence mechanism, not a legitimate provisioning event.", severity: "Critical", impact: 5, likelihood: 2, triggeredAt: "2026-06-12T03:14:00Z" },

  // ── E · Exfiltration & Lateral Movement (Collection, Exfiltration, Impact) ─
  { id: "mf-031", technique: "T1021", name: "Remote Services", tactic: "Lateral Movement", ptraceCategory: "E", identityId: "id-001", identityName: "jane.doe", description: "Once inside role-deploy-box, jane.doe's session can pivot onward into svc-github-actions' deploy-prod-role — the first hop of a lateral chain, not just an escalation endpoint, per Unified Impact Analysis's choke-point mapping.", severity: "Critical", impact: 5, likelihood: 4, triggeredAt: "2026-07-31T08:10:00Z" },
  { id: "mf-032", technique: "T1530", name: "Data from Cloud Storage Object", tactic: "Collection", ptraceCategory: "E", identityId: "id-104", identityName: "svc-orphaned-etl", description: "svc-orphaned-etl can read raw PII directly from GCS with no owner to notice the access — Identity Exposure Map already flags this as the tenant's highest-sensitivity reachable resource for an orphaned identity.", severity: "Critical", impact: 5, likelihood: 3, triggeredAt: "2026-07-10T03:05:00Z" },
  { id: "mf-033", technique: "T1486", name: "Data Encrypted for Impact", tactic: "Impact", ptraceCategory: "E", identityId: "id-101", identityName: "svc-payments-api", description: "svc-payments-api can disable the KMS key grants sibling payments services depend on — a blast radius Unified Impact Analysis already marked as systemic, not local, to this one identity.", severity: "Critical", impact: 5, likelihood: 2, triggeredAt: "2026-07-29T21:10:00Z" },
  { id: "mf-034", technique: "T1537", name: "Transfer Data to Cloud Account", tactic: "Exfiltration", ptraceCategory: "E", identityId: "id-107", identityName: "svc-billing-sync", description: "svc-billing-sync can copy finance tables to a project outside the tenant boundary — the same Azure Storage Blob Data Reader grant Access Discovery flagged, now scored for what it would enable next.", severity: "Medium", impact: 3, likelihood: 2, triggeredAt: "2026-07-20T14:20:00Z" },
  { id: "mf-035", technique: "T1530", name: "Data from Cloud Storage Object", tactic: "Collection", ptraceCategory: "E", identityId: "id-004", identityName: "tom.walker", description: "A burst of KMS Decrypt calls from tom.walker outside normal review hours doesn't yet show exfiltration, but the resource reached — prod-key-ring — is the same critical asset his Access Discovery grant already surfaced.", severity: "Medium", impact: 3, likelihood: 3, triggeredAt: "2026-07-24T02:15:00Z" },
  { id: "mf-036", technique: "T1485", name: "Data Destruction", tactic: "Impact", ptraceCategory: "E", identityId: "id-114", identityName: "svc-finance-reporter", description: "svc-finance-reporter has retained delete permissions on finance-reports since alice.brooks's departure — Impact tactic satisfied even without an active trigger, because deletion capability alone is the exposure.", severity: "Medium", impact: 3, likelihood: 1, triggeredAt: "2026-05-28T00:00:00Z" },

  // ── Additional stage coverage — varies per-stage counts away from a flat 6 ──
  { id: "mf-037", technique: "T1069", name: "Permission Groups Discovery", tactic: "Discovery", ptraceCategory: "P", identityId: "id-024", identityName: "marcus.vale", description: "marcus.vale's SRE tooling enumerates IAM group membership org-wide on a recurring schedule, well beyond the on-call scope it was provisioned for.", severity: "Low", impact: 1, likelihood: 3, triggeredAt: "2026-07-19T08:00:00Z" },
  { id: "mf-038", technique: "T1613", name: "Container and Resource Discovery", tactic: "Discovery", ptraceCategory: "P", identityId: "id-014", identityName: "nora.whitfield", description: "nora.whitfield's finance-side automation lists cluster workloads outside her department's namespace boundary.", severity: "Low", impact: 1, likelihood: 2, triggeredAt: "2026-07-14T09:30:00Z" },
  { id: "mf-039", technique: "T1548.003", name: "Sudo and Sudo Caching", tactic: "Privilege Escalation", ptraceCategory: "R", identityId: "id-010", identityName: "mei.lin", description: "mei.lin's cached sudo session on a shared SRE bastion outlives the ticket that authorized it, granting root well past the approved window.", severity: "Medium", impact: 3, likelihood: 3, triggeredAt: "2026-07-27T05:00:00Z" },
  { id: "mf-040", technique: "T1078.003", name: "Local Accounts", tactic: "Privilege Escalation", ptraceCategory: "R", identityId: "id-012", identityName: "fatima.hassan", description: "fatima.hassan's local admin account on a shared product-review VM was never scoped to her Product-team grant, and now assumes broader host-level rights than her cloud identity carries.", severity: "High", impact: 4, likelihood: 2, triggeredAt: "2026-07-24T13:00:00Z" },
  { id: "mf-041", technique: "T1611", name: "Escape to Host", tactic: "Privilege Escalation", ptraceCategory: "R", identityId: "id-016", identityName: "henry.cole", description: "henry.cole's containerized finance-reporting job can escape to the underlying node, escalating a namespace-scoped grant into host-level access.", severity: "Critical", impact: 5, likelihood: 2, triggeredAt: "2026-07-30T22:00:00Z" },
  { id: "mf-042", technique: "T1055", name: "Process Injection", tactic: "Privilege Escalation", ptraceCategory: "R", identityId: "id-019", identityName: "quinn.adebayo", description: "quinn.adebayo's DevOps agent injects into a privileged sibling process on the same host, inheriting rights never granted to the agent directly.", severity: "Medium", impact: 3, likelihood: 2, triggeredAt: "2026-07-17T18:20:00Z" },
  { id: "mf-043", technique: "T1078.001", name: "Default Accounts", tactic: "Initial Access", ptraceCategory: "A", identityId: "id-023", identityName: "sofia.reyes", description: "sofia.reyes's platform tooling still authenticates through a default vendor-shipped service account that was never rotated to a named identity.", severity: "Medium", impact: 3, likelihood: 2, triggeredAt: "2026-07-16T10:00:00Z" },
  { id: "mf-044", technique: "T1070", name: "Indicator Removal", tactic: "Defense Evasion", ptraceCategory: "C", identityId: "id-017", identityName: "maya.singh", description: "maya.singh's finance workstation clears cloud-trail-adjacent local logs after each session, consistent with concealment rather than routine cleanup.", severity: "Medium", impact: 3, likelihood: 2, triggeredAt: "2026-07-21T06:45:00Z" },
  { id: "mf-045", technique: "T1578", name: "Modify Cloud Compute Infrastructure", tactic: "Defense Evasion", ptraceCategory: "C", identityId: "id-018", identityName: "elise.moran", description: "elise.moran's data-pipeline role can reconfigure compute infrastructure to mask its own resource footprint from routine audit sweeps.", severity: "High", impact: 4, likelihood: 2, triggeredAt: "2026-07-11T15:10:00Z" },
  { id: "mf-046", technique: "T1556.006", name: "Multi-Factor Authentication", tactic: "Credential Access", ptraceCategory: "C", identityId: "id-022", identityName: "helena.cho", description: "helena.cho's MFA enrollment was disabled during a support ticket and never re-enabled, leaving a standing concealment path for anyone holding her credential.", severity: "Low", impact: 2, likelihood: 2, triggeredAt: "2026-06-25T00:00:00Z" },
];
