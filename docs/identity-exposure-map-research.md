...
3. **The PRD's central argument is undemonstrable on the current dataset, by a factor of eight.** §1 rests on "forty low-sensitivity Direct paths and one Hop path into a production database are not the same story, and a table of forty-one rows doesn't make that difference legible." The largest footprint in the seed is **5 paths** (`agent-support-triage`); the median is **1**. There is no identity for which a table is illegible, so there is currently nothing for the map to be better *than*. Beat 1 of §9 exists to fix this and is the single highest-value piece of seed work in this module.
...
Beats 24–29, following Access Discovery's beats 19–23. The narrative spine: *Access Discovery told you every route. Exposure Map tells you which identity to worry about — and the answer is not the one with the most routes.*

| Beat | Identity | What the CISO sees | Why it is needed |
|---|---|---|---|
| 24 | new, wide-but-shallow (≈40 low-sensitivity direct paths) | Largest table in the demo, ~7/100. "Forty rows. Nothing here." | The counterexample the module rests on (§1.3). Without it the score is indistinguishable from a row count. |
| 25 | `user-jane` (existing) | Four rows, **78/100**, 83% from one hop | The comparison lands: fewer rows, ten times the score. Uses §5's worked example verbatim. |
| 26 | new, unclassified-heavy | Distinct hatched nodes, completeness metric visible | Makes §3.2's honest answer visible instead of hidden. The "we don't fake what we don't know" beat. |
| 27 | new, `hop` + `indirect` on one permission | Multi-route badge; closing the hop does not clear it | Answers PRD §8 question 2 on screen (§4.5). |
| 28 | `agent-support-triage` (existing, **unmodified**) | Outermost ring at distance 6, across two systems | The deepest chain in the estate. Requires per-distance rings (§4.1) to be visible at all. |
| 29 | Capital One slide | 1 path, 700 buckets, 77 days | The real-world anchor (§3.4), and the moment to say the false-positive rate is undefined. |

**Beat 24 is the one to build first.** It is also the one most likely to disturb the pinned assertions, so it is scoped in STEP 4 before any code is written.

**Numbers to re-record here after the seed changes** (current values, `d31f0fd`): ownership queue 24; access `{direct 66, indirect 66, hop 6}`; `identities_with_hop` 4; `identities_scanned` 115; 123 identities / 8 groups / 36 permissions / 9 sensitive / 7 apps.
...
- **What is the exposure score of an identity with zero paths?** 21 of 115 non-group identities have none. `0` and "not applicable" are different claims, and rule 7 says they must not collapse. Leaning toward a discriminated `{ kind: 'no_paths' }` rather than a zero that sorts alongside real zeros.
